...
#
# A sessão recebe a ordem de abrir a PR, mas uma sessão que bate no --max-turns ou que
# se enrola no fim deixa a branch publicada e nenhuma PR. Isso pararia o encadeamento
# em silêncio, que é o pior jeito de parar: abrir a PR aqui faz o trabalho chegar ao
# portão de teste de qualquer jeito.
set -euo pipefail
...
# O "Closes #n" é o que fecha a issue na mesclagem. Se a sessão abriu a PR sem ele, a
# issue ficaria aberta com o trabalho já mesclado — então ele é acrescentado aqui.
