...
Apps built on Trilha and running for real — the examples show the idioms, these show the scale.

| App | What it is | What it exercises |
|---|---|---|
| [Partiu](https://partiu.eoslab.com.br) | Group-travel PWA (shared expenses with receipts, settlement, itinerary, lists with AI, MCP server). Ported from Next.js to Go in 2026; the port is measured screen by screen against the original (visible text, accessibility tree, headers, pixels, in WebKit and Chromium). | 102 routes generated by `trilha gen` from the `servidor/app/**` tree (per-folder `middleware.go`, `layout.go`, `error.go`, `not_found.go`), `KindPage` islands, `NoTimeout` for photo uploads, `ErrNotFound` as the 404 a non-member gets, its own CSP-with-nonce headers next to the framework's, and an MCP server on the same session. The adoption report, with the ten frictions it found and what each one changed here, is [DEMANDA-PARTIU.md](DEMANDA-PARTIU.md). |
| Farol | Shared back office for the apps above (CRM, funnel, tracking pixel, policies, alerts). | `LayoutFunc`, `CSRFInput`, `Off` for the pieces it does not want, the pixel served to other origins. |

Both are private repositories; the report is public because it is about Trilha.
