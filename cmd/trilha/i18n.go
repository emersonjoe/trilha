...
`},
	...
	"with needs":       {"template %q needs recipe %q; adding it automatically\n", "o template %q precisa da receita %q; acrescentando automaticamente\n"},
	"bad recipe":       {"unknown recipe %q; valid recipes: %s", "receita desconhecida %q; receitas válidas: %s"},
	...
	"flag crud policy": {"guard the generated folder with this permission module at the administer level", "protege a pasta gerada com este módulo de permissão no nível administrar"},
	"flag crud schema": {"render the forms compactly with trilha.Schema and ui.SchemaForm", "desenha os formulários de forma compacta com trilha.Schema e ui.SchemaForm"},
	"crud add store":   {"run `trilha add store` first: it owns the dialect, the migrations and the query kit this would be written against", "rode `trilha add store` antes: é dela o dialeto, as migrações e o kit de consulta contra os quais isto seria escrito"},
	...
	"generate usage":   {"usage: trilha generate page|route|test <url> | component <Name> | crud <Type> [--at dir] [--store sqlite|postgres] [--tenant] [--policy module] [--schema] [--methods GET,POST] [--bind Type] [--form Type] [--layout file] [--force] [--dir path] [--lang en|pt]", "uso: trilha generate page|route|test <url> | component <Nome> | crud <Tipo> [--at dir] [--store sqlite|postgres] [--tenant] [--policy módulo] [--schema] [--methods GET,POST] [--bind Tipo] [--form Tipo] [--layout arquivo] [--force] [--dir caminho] [--lang en|pt]"},
	...
	"crud missing schema":         {"is not in the form schema", "não está no schema do formulário"},
