...
| Login of your own | `examples/local-login` | session without a provider (`auth.Sessions`, PBKDF2); the two ways to an API that already exists — `Config.Upstreams` forwarding `/api/` for what the browser fetches, and a screen rendered on the server through the client `trilha client` generates — both carrying the session's credential and never the browser's |
