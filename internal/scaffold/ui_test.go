...
func TestWriteUIStamp(t *testing.T) {
	...
	if err != nil || len(res) != 3 || res[1].Action != "criado" {
		t.Fatal(err, res)
	}
	...
	res, err = WriteUI(dir, false, false, false)
	if err != nil || res[1].Action != "atualizado" {
		t.Fatal(err, res)
	}
	...
	if res, err = WriteUI(dir, true, true, false); err != nil || res[1].Action != "atualizado" || len(res) != 2 {
		t.Fatal(err, res)
	}
	...
	if b, _ := os.ReadFile(theme); string(b) != ":root{}" || res[0].Action != "mantido (seu tema)" {
