...
**<https://emersonjoe.github.io/trilha/pt>** — trilha "Aprender" (do `trilha new` ao deploy, com
desafios) e "Referência" por pacote. O site é um app Trilha, exportado com `trilha export`, em inglês (`/`) e português (`/pt`).

## Começando

```bash
go install github.com/emersonjoe/trilha/cmd/trilha@latest
trilha new meu-app --lang pt && cd meu-app   # --lang pt gera os textos em português
...
Contribuições são bem-vindas: veja [CONTRIBUTING](docs/pt-BR/CONTRIBUTING.md), o
[código de conduta](docs/pt-BR/CODE_OF_CONDUCT.md), a [política de segurança](docs/pt-BR/SECURITY.md) e a
[governança](docs/pt-BR/GOVERNANCE.md) (originais em inglês na raiz). Mudanças de comportamento seguem o fluxo spec-kit em `specs/`.
