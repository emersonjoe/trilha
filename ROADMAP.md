...
| Segurança | CSP com nonce, HSTS, COOP, `Permissions-Policy`, proxies confiáveis, timeouts, limite de corpo, upload com tipo pelo conteúdo (e documento com script servido inline só sob `default-src 'none'`), CORS, `trilha audit`, segredo selado nas `Connections`, trilha de auditoria | specs 004, 024, 028, 029, 120, 149 |
...
28. ~~[#50](https://github.com/emersonjoe/trilha/issues/50) **`trilha mcp` e MCP de documentação.**~~ **Entregue na 0.134.0** (spec 155): além do wrapper local já existente, o próprio site serve `POST /mcp` somente leitura com `search_docs`, `get_page` e `get_recipe`; o export para Pages continua estático.
...
O épico [#119](https://github.com/emersonjoe/trilha/issues/119) foi **concluído na 0.134.0**. A tese da fase não é
...
86. ~~[#118](https://github.com/emersonjoe/trilha/issues/118) Erros de runtime que ensinam.~~ **Concluída na 0.134.0** (spec 155): depois de `Hint`, relatórios do navegador e auditoria estática, o catálogo único agora publica `/docs/errors/<código>` e o bench mede o cenário “corrija este erro”.
...
97. ~~[#118](https://github.com/emersonjoe/trilha/issues/118) Relatos de CSP, fragmento e flash em desenvolvimento.~~ **Concluída na 0.134.0**; a 0.78.0 entregou o canal e a 0.134.0 fechou catálogo e benchmark.
98. ~~[#118](https://github.com/emersonjoe/trilha/issues/118) Auditoria estática dos erros que o código já revela.~~ **Concluída na 0.134.0**; a 0.79.0 entregou as regras e a 0.134.0 fechou os dois itens finais.
99. ~~[#115](https://github.com/emersonjoe/trilha/issues/115) `trilha generate crud <Tipo>`.~~ **Concluída na 0.134.0** (spec 155): `--tenant`, `--policy` e `--schema` completam o gerador; o bench agora mede “adicione um cadastro de X”.
...
110. ~~[#147](https://github.com/emersonjoe/trilha/issues/147) Todo app de negócio acaba com uma caixa de coisas que alguém precisa aprovar — 464 linhas de tela e 220 de rota, medidas no Acervo, para um padrão que se repete.~~ **Entregue na 0.92.0** (spec 113): o pacote `approval` (abrir, decidir, o gancho do que a decisão significa, prazo que vence sozinho, quem pode decidir conferido no pacote), o `ui.Inbox` e o `ui.InboxBadge`, e a receita `trilha add approvals`. **[#250](https://github.com/emersonjoe/trilha/issues/250) — as três formas que a fila do Acervo tinha e não cabiam num voto único, entregue na 0.135.0** (spec 156): `Request.Quorum`/`Record.Votes` para decisão por comitê, `ui.InboxRow.Action` para a linha cuja ação não é aprovar/rejeitar, `ui.InboxRow.Progress` para "1/3", e o `display` do `ui.SchemaForm` desenhando o `Label`.
